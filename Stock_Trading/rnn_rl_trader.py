...
class RNNModel:
    ...
    def __init__(self, input_dim, n_action, lstm_units=64, dense_units=64, learning_rate=0.01):
        # Define the model architecture with LSTM and Bi-directional layers
        self.model = tf.keras.Sequential([
            tf.keras.layers.InputLayer(input_shape=(None, input_dim)),  # Input shape for sequences
            tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(lstm_units, activation='tanh', return_sequences=True)),
...
class DQNAgent(object):
    ...
    def __init__(self, state_size, action_size, lstm_units=64, dense_units=64, learning_rate=0.01):
        ...
        self.model = RNNModel(state_size, action_size, lstm_units=lstm_units, dense_units=dense_units, learning_rate=learning_rate)
    ...
    def train(self, state, action, reward, next_state, done):
        ...
        # Train the RNNModel using Stochastic Gradient Descent
    ...
    def load(self, name):
        # Load weights for the RNNModel
        self.model.load_weights(name)

    def save(self, name):
        # Save weights for the RNNModel
...
def play_one_episode(agent, env, is_train):
    ...
    limit = 0
    # Initialize an empty sequence for LSTM input
    sequence = np.zeros((1, 1, state.shape[1]))  # Add the time dimension
    # While it's not done:
    while not done:
        limit = limit + 1
        action = agent.act(sequence)  # given a state, give me an action
        ...
        next_state = np.reshape(next_state, (1, 1, -1))  # reshape to include the time dimension
        sequence = np.vstack((sequence, next_state))  # add the next state to the sequence

        if is_train == 'train':  # if we want also to train the model...
            # Train the RNNModel using Stochastic Gradient Descent
            # Reshape the sequence to have three dimensions
            agent.train(np.reshape(sequence[:-1], (1, -1, state.shape[1])), 
                        action, 
                        reward, 
                        np.reshape(sequence[1:], (1, -1, state.shape[1])), 
                        done)
