...
"""
we'll be looking at how to implement Monte carlo for control problem  with exploring starts (find the best policy)
with exploring starts = random start each game 
"""
...
def play_game(grid, policy, max_steps=20):
  ...
  # Set random starting state (start in a random location)
  start_states = list(grid.actions.keys()) # states
  start_idx = np.random.choice(len(start_states)) # random choise state id
  grid.set_state(start_states[start_idx]) # set the state on the grid
